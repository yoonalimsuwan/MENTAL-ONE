...
def soft_clamp(x: torch.Tensor, lo: float, hi: float) -> torch.Tensor:
    ...
    Uses tanh-based smooth projection:  f(x) = lo + (hi-lo) * sigmoid((x-lo)/(hi-lo)*6)
    Gradient exists everywhere, unlike hard clamp which zeros grad at boundaries.
    """
    center = (hi + lo) / 2.0
    scale  = (hi - lo) / 2.0
    return center + scale * torch.tanh((x - center) / (scale + 1e-8))
...
class PSYONEBridge(nn.Module):
    ...
    def _setup_mental_one_bridge(self) -> None:
        ...
        try:
            self._ssc: Optional[SSCClassifier]      = self.engine.ssc
            self._soc: Optional[SOCController]      = self.engine.soc
    ...
    def forward(
        ...
        if HAS_MENTAL_ONE and self._ssc is not None:
            try:
                eeg_for_mental = sensory_state.to(self.device).float()
                if eeg_for_mental.dim() == 1:
                    eeg_for_mental = eeg_for_mental.unsqueeze(0).unsqueeze(0)
                diagnosis, _ = self._ssc.classify(eeg_for_mental)
                state.diagnosis = diagnosis
                if self._designer is not None:
                    state.intervention_plan = self._designer.design(
                        diagnosis, state.to_dict()
                    )
            except Exception as e:
