...
#                             one_core_mental integration (v3), cross-ecosystem
#                             CahnHilliardMentalBridge architecture
...
# Priority order:
#   0. one_core_mental  — MENTAL ONE ecosystem  (provides CahnHilliardMentalBridge
#                         + structural_biharmonic_n when DNS core unavailable)
...
# ── Priority 0: one_core_mental  (always attempt — provides CahnHilliardMentalBridge)
_HAS_ONE_CORE_MENTAL = False
CahnHilliardMentalBridge = None  # type: ignore[assignment]
try:
    from one_core_mental import (
        ...
        structural_biharmonic_n as _biharmonic_mental,
        soft_clamp as _soft_clamp_mental,
        MENTAL_VERSION,
    )
    _HAS_ONE_CORE_MENTAL = True
except ImportError:
    _biharmonic_mental = None
    _soft_clamp_mental = None
...
try:
    ...
    # Use one_core_mental biharmonic if available, else None
    _biharmonic_core = _biharmonic_mental if _HAS_ONE_CORE_MENTAL else None
    ...
    # Standalone fallbacks for abstract bases — only if one_core_mental
    # did not already supply them
    if not _HAS_ONE_CORE_FOLD and not _HAS_ONE_CORE_MENTAL:
...
class StructuralCahnHilliard3D(nn.Module):
    ...
    def extra_repr(self) -> str:
        ...
        return (f"scheme={c.scheme}, lap={c.laplacian}, "
                ...
                f"one_core_mental={'yes' if _HAS_ONE_CORE_MENTAL else 'no'}")
