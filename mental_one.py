...
class InterventionDesigner:
    ...
    def design_plan(self, diagnosis, current_state, desired_state):
        plan = {
            'medication': random.sample(self.drugs.get(diagnosis, ['General support']),
                                        min(3, len(self.drugs.get(diagnosis, [])))),
            'psychotherapy': random.sample(self.therapies.get(diagnosis, ['Supportive counselling']),
                                           min(2, len(self.therapies.get(diagnosis, [])))),
