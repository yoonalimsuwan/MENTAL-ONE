...
class BrainStateInterfaceDetector(InterfaceDetectorBase):
    """
...
class LangevinBrainIntegrator(nn.Module):
    ...
    def _adaptive_T_gamma(self, s: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        T = soft_clamp(T, self.base_temp * 0.5, self.base_temp * self.temp_boost_factor)  # differentiable
    ...
    def baoa_step(
        ...
        Returns:
            ...
            T        : scalar tensor — adaptive temperature (differentiable).
            sigma    : scalar tensor — SSC stress (differentiable).
        """
...
class LangevinSOCEvolve(nn.Module):
    ...
    def forward(self, s0: torch.Tensor, steps: int = 20) -> torch.Tensor:
        ...
        for _ in range(steps):
            ...
            s = soft_clamp(s_new.detach(), 0.0, 1.0)  # differentiable boundary
...
class LangevinItoStep(nn.Module):
    ...
    def step(self, x: torch.Tensor) -> torch.Tensor:
        ...
        noise_scale = torch.sqrt(torch.tensor(0.001987, device=x.device) * T)
        ...
        return soft_clamp(x_next, 0.0, 1.0)  # differentiable boundary
...
class LangevinMentalEvolution(nn.Module):
    ...
    def forward(self, mu_seq: torch.Tensor, steps: int = 50) -> dict:
        ...
        mu_smooth = soft_clamp(mu_smooth, 0.0, 1.0)  # differentiable boundary
...
if __name__ == "__main__":
    ...
    evolve = LangevinSOCEvolve(clf, target_disorder='MDD')
