...
class InterfaceDetector(InterfaceDetectorBase):
    ...
    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        # (O(N^2) memory). This call is numerically IDENTICAL to the
        # original formula for small N and switches to a memory-safe,
        # still-exact chunked computation once N is large — see
...
# SemanticStateContraction imported from one_core_mental
...
class CSOCThermostat(CSOCBase):
    ...
    def __init__(
        ...
        """
        super().__init__(sigma_target=sigma_target, epsilon_fp=epsilon_fp, boost_factor=temp_boost_factor)
...
class AdvancedStructuralLangevin(nn.Module):
    ...
    def baoa_step(
        ...
        Returns:
            ...
            T        : scalar tensor — adaptive temperature (K), differentiable.
            sigma    : scalar tensor — SSC-filtered stress (Å), differentiable.
        """
    ...
    def full_step(
        ...
        Returns:
            new_coords     : (N, 3)
            new_velocities : (N, 3)
            T_scalar       : scalar tensor — adaptive temperature (differentiable)
            sigma_scalar   : scalar tensor — SSC stress (differentiable)
        """
        N = coords.shape[0]
...
if __name__ == "__main__":
    ...
    for step in range(5):
        ...
        print(f"  {step:>5}  {T_sc.item():>8.2f}  {sigma_sc.item():>8.4f}  "
